...
describe('Resend webhook verification', () => {
	it('loads both account secrets and keeps the legacy fallback', () => {
		...
		})).toEqual(['whsec_polyurea', 'whsec_coating', 'whsec_legacy']);
	});
	...
	it('removes duplicate and empty secrets', () => {
		expect(configuredWebhookSecrets({
			...
			resend_webhook_secret: 'whsec_same'
		})).toEqual(['whsec_same']);
	});
	...
	it('accepts an event signed by the second Resend account', () => {
		...
		const body = verifyWebhookWithSecrets(
			...
			['whsec_polyurea', 'whsec_coating', 'whsec_legacy']
		);
